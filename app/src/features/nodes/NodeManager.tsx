...
export default function NodeManager() {
  ...
  return <section className="workspace-overlay" aria-label="执行节点管理">
    <header><div><span>运行基础设施</span><h1>执行节点</h1><p>连接本机或远程执行节点，凭据只写不回读。</p></div></header>
    <div className="overlay-grid">
      <form className="settings-card form-grid" onSubmit={submit}>
        <h2>节点配置</h2>
        <label htmlFor="node-id">节点标识</label><input id="node-id" value={form.id} onChange={field('id')} required />
        ...
        <label htmlFor="ssh-host">远程连接主机地址</label><input id="ssh-host" value={form.sshHost} onChange={field('sshHost')} />
        <label htmlFor="ssh-port">远程连接端口</label><input id="ssh-port" type="number" min="1" max="65535" value={form.sshPort} onChange={field('sshPort')} />
        <label htmlFor="ssh-user">远程连接用户</label><input id="ssh-user" value={form.sshUser} onChange={field('sshUser')} />
        <label htmlFor="ssh-key">远程连接私钥路径</label><input id="ssh-key" value={form.sshKeyPath} onChange={field('sshKeyPath')} />
        <label htmlFor="node-token">节点访问密钥</label><input id="node-token" type="password" value={form.credential} onChange={field('credential')} autoComplete="off" />
      ...
      <div className="settings-card node-list"><h2>已连接节点</h2>{nodes.length === 0 ? <p className="muted">尚无节点。</p> : nodes.map((node) => <article key={node.id}>
        ...
        {node.credential_configured === true && <p className="configured">访问密钥已配置</p>}
        <button type="button" onClick={() => void diagnoseNode(node.id)}><RefreshCw size={14} />重新诊断</button>

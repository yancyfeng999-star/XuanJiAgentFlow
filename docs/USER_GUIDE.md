...
2. **就绪检查**：画布上方的「执行就绪检查」列出项目目录、Planner、工作流审核、任务匹配、执行节点、凭据六项状态；每个阻塞项有直达修复入口。
3. **规划**：在画布输入目标 →「生成规划」。Planner 需在「设置」配置 base_url / model / API Key。
...
- 桌面壳（sidecar 模式）下 Planner Key 与 Node Token 存入 macOS Keychain；从旧 `credentials.json` 迁移时先验证读回再删除旧文件，失败保留旧数据。开发模式默认使用 `credentials.json`（权限 `0600`），API 只返回是否已配置，不回传密钥。

...
a = Analysis(
    ...
    binaries=[],
    datas=[
        (str(backend_root.parent / "node-agent" / "app.py"), "node-agent"),
        (str(backend_root.parent / "node-agent" / "executor.py"), "node-agent"),
        (str(backend_root.parent / "node-agent" / "pyproject.toml"), "node-agent"),
    ],
    hiddenimports=[
        "uvicorn",
        "uvicorn.logging",
